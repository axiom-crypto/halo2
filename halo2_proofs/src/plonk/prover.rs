...
#[derive(Clone)]
struct AdviceCommitted<C: CurveAffine, B: Basis> {
    ...
    T: TranscriptWrite<C, E>,
{
    ...
    advice_single: AdviceCommitted<C, LagrangeCoeff>,
...
/// challenge-squeeze steps [`create_proof`] performs internally — and returns
/// the resulting instance/advice singles and per-phase challenges without
/// continuing to lookups, permutations, or the multiopen argument.
///
/// The transcript is left in the same state [`create_proof`] would produce at
/// the boundary (vk hashed, instances absorbed, per-phase advice commitments
/// written, per-phase challenges squeezed). Intended as a diagnostic entry
/// point — comparing an out-of-band witness generator's advice against
/// `Circuit::synthesize` — not for producing verifiable proofs.
///
